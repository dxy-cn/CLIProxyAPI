...
func TestBuildConfigChangeDetails_FlagsAndKeys(t *testing.T) {
	oldCfg := &config.Config{
		...
		QuotaWarning:           config.QuotaWarning{WebhookURL: "", Threshold: 10},
		ClaudeKey:              []config.ClaudeKey{{APIKey: "c1"}},
		CodexKey:               []config.CodexKey{{APIKey: "x1"}},
		AmpCode:                config.AmpCode{UpstreamAPIKey: "keep", RestrictManagementToLocalhost: false},
		RemoteManagement:       config.RemoteManagement{DisableControlPanel: false, PanelReleaseURL: "old/repo", SecretKey: "keep"},
		SDKConfig: sdkconfig.SDKConfig{
			RequestLog:                 false,
			ProxyURL:                   "http://old-proxy",
	...
	newCfg := &config.Config{
		...
		QuotaWarning:           config.QuotaWarning{WebhookURL: "https://qyapi.weixin.qq.com/cgi-bin/webhook/send?key=secret", Threshold: 20},
		ClaudeKey: []config.ClaudeKey{
	...
	expectContains(t, details, "quota-warning.webhook-url: created")
	expectContains(t, details, "quota-warning.threshold: 10 -> 20")
	...
	expectContains(t, details, "remote-management.disable-auto-update-panel: false -> true")
	expectContains(t, details, "remote-management.panel-release-url: old/repo -> new/repo")
...
func TestBuildConfigChangeDetails_AllBranches(t *testing.T) {
	...
	expectContains(t, changes, "quota-warning.webhook-url: created")
	expectContains(t, changes, "quota-warning.threshold: 0 -> 20")
	...
	expectContains(t, changes, "remote-management.panel-release-url: old/repo -> new/repo")
