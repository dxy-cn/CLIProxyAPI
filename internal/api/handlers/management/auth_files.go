		...
		if info, errInfo := e.Info(); errInfo == nil {
			...
			if data, errRead := os.ReadFile(full); errRead == nil {
				...
				if wv := gjson.GetBytes(data, "websockets"); wv.Exists() {
					if websockets, errParse := strconv.ParseBool(strings.TrimSpace(wv.String())); errParse == nil {
...
func authFileWebsockets(auth *coreauth.Auth) (bool, bool) {
	...
	if auth.Attributes != nil {
		if value, ok := authFileBoolValue(auth.Attributes["websockets"]); ok {
			return value, true
		}
	}
	if auth.Metadata != nil {
		if value, ok := authFileBoolValue(auth.Metadata["websockets"]); ok {
			return value, true
		}
	}
	return false, false
