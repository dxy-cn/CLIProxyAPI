...
func (h *Handler) maybeSendCodexQuotaWarning(ctx context.Context, auth *coreauth.Auth, payload gin.H) {
	...
	dedupeKey := h.quotaWarningDedupeKey(auth, window, threshold, webhookURL)
	...
	content := buildQuotaWarningContent(auth, window)
...
func (h *Handler) shouldDispatchQuotaWarningsAfterConfigChange(oldCfg *config.Config, newCfg *config.Config) bool {
	...
	if oldCfg != nil {
		...
		oldWebhookURL = strings.TrimSpace(oldCfg.QuotaWarning.WebhookURL)
	}
	newThreshold := newCfg.QuotaWarning.Threshold
	newWebhookURL := strings.TrimSpace(newCfg.QuotaWarning.WebhookURL)
	if oldEnabled && oldThreshold == newThreshold && oldWebhookURL == newWebhookURL {
		return false
	}
...
func (h *Handler) startQuotaWarningScanner(ctx context.Context, interval time.Duration) {
	...
	go func() {
		if h.quotaWarningDone != nil {
			defer close(h.quotaWarningDone)
		}
...
func (h *Handler) runQuotaWarningScan(ctx context.Context) {
	...
	if h.quotaWarningRunning {
		h.quotaWarningScanMu.Unlock()
		return
	}
	h.quotaWarningRunning = true
	h.quotaWarningScanMu.Unlock()

	defer func() {
		h.quotaWarningScanMu.Lock()
		h.quotaWarningRunning = false
		h.quotaWarningScanMu.Unlock()
	}()
...
func lowestCodexQuotaWindow(payload gin.H) (quotaWarningWindow, bool) {
	...
	for _, window := range windows {
		...
		if lowest.ID == "" || window.Remaining < lowest.Remaining {
			lowest = window
		}
	}
	if lowest.ID == "" {
...
func buildQuotaWarningContent(auth *coreauth.Auth, window quotaWarningWindow) string {
...
func (h *Handler) quotaWarningDedupeKey(auth *coreauth.Auth, window quotaWarningWindow, threshold int, webhookURL string) string {
	identity := ""
	if auth != nil {
	...
	return strings.Join([]string{identity, window.ID, window.DedupeReset, strconv.Itoa(threshold), quotaWarningWebhookKey(webhookURL), strconv.FormatInt(version, 10)}, "|")
}

func quotaWarningWebhookKey(webhookURL string) string {
...
func sendWeComQuotaWarning(ctx context.Context, webhookURL string, content string) error {
	...
	if err != nil {
		return fmt.Errorf("build wecom request: %s", redactedWeComWebhookError(webhookURL, err))
	}
	req.Header.Set("Content-Type", "application/json")

	resp, err := http.DefaultClient.Do(req)
	if err != nil {
		return fmt.Errorf("post wecom webhook: %s", redactedWeComWebhookError(webhookURL, err))
	}
...
func redactedWeComWebhookError(webhookURL string, err error) string {
	if err == nil {
		return ""
	}
	message := err.Error()
	return strings.ReplaceAll(message, strings.TrimSpace(webhookURL), redactedWeComWebhookURL(webhookURL))
}

func redactedWeComWebhookURL(webhookURL string) string {
