...
CLI Proxy API 是一个 Go 后端服务，用一个代理入口承接多种 AI 客户端协议。它可以接收 OpenAI 兼容的 Chat Completions、Completions、Images、Videos、Responses，请求 Claude Messages、Gemini 原生接口、Codex Responses、Codex WebSocket 流量、xAI 图片/视频调用，以及 Amp CLI 的 provider 调用，再把请求路由到配置好的上游账号、API Key、OAuth token 或 OpenAI 兼容服务。

服务内部共用一套运行时，负责认证、模型注册、协议转换、重试、凭证调度、请求日志、使用量统计和管理 API。

## 主要能力

- 为 OpenAI、Claude、Gemini、Codex、xAI 和 Amp 兼容客户端提供统一 API 入口。
- 支持多种上游凭证：OAuth token 文件、服务商 API Key、Vertex 服务账号、OpenAI 兼容服务、xAI OAuth 凭证和 Amp 上游密钥。
...
- 支持 OpenAI 兼容的图片和视频接口，包括 xAI Grok 图片和视频模型。
- 支持 Codex Responses WebSocket 转发，并可开启 WebSocket 鉴权。
- 提供管理 API 和 Web 控制台，可管理配置、auth 文件、API Key、API Key 使用量、模型监控数据、usage queue、日志和路由设置。
- 支持请求日志、滚动应用日志、健康检查、可选 pprof、Redis 风格 usage queue 和使用量持久化。
...
| xAI | xAI OAuth auth 文件 | Chat、图片和视频执行，包括 Grok 图片/视频模型和 OAuth callback 流程 |
| OpenAI 兼容服务 | `openai-compatibility` 配置 | 可配置 base URL、API Key、headers、模型 alias、compact responses 和支持图片能力的模型 |
...
| `POST /v1/videos` | 由 xAI Grok 视频模型支持的 OpenAI 兼容视频创建 |
| `POST /v1/videos/generations` | xAI 视频生成接口 |
| `POST /v1/videos/edits` | xAI 视频编辑接口 |
| `POST /v1/videos/extensions` | xAI 视频扩展接口 |
| `GET /v1/videos/:request_id` | 查询 xAI 视频请求状态或结果 |
...
| `disable-image-generation` | 全局禁用图片工具注入，或只在 chat 风格接口禁用 |
...
| `remote-management` | 管理密钥、远程访问和控制台资源设置 |
| `remote-management.disable-control-panel` | 禁用内置管理面板路由和面板资源下载 |
| `remote-management.disable-auto-update-panel` | 禁用后台管理面板资源自动更新 |
| `remote-management.panel-release-url` | 覆盖管理面板 release 资源地址 |
...
`disable-image-generation` 支持 `false`、`true` 或 `"chat"`。`true` 会禁用 `image_generation` 注入，并让 `/v1/images/*` 接口返回 `404`；`"chat"` 只在非图片接口禁用图片生成注入，同时保留图片接口可用。
...
- API Key 使用量汇总和实时 usage queue 记录；
- OAuth 辅助流程，包括 xAI；
- 模型和 quota 监控数据；
- 请求日志和使用量统计；
- 路由策略和 WebSocket 鉴权设置；
- 面向指定 API Key 的公开监控接口；
- Amp 配置和监控元数据。
...
启用 Home control-plane 模式时，服务可以通过 `--home-jwt` 或 `HOME_JWT` 接收 Home 凭证。`--home-disable-cluster-discovery` 会保持当前配置的 endpoint，不再自动发现 cluster endpoint。
...
| `internal/runtime/executor` | Codex、Claude、Gemini、Vertex、Antigravity、Kimi、xAI 和 OpenAI 兼容上游 executor |
| `internal/home` | 可选 Home control-plane 集成 |
| `internal/redisqueue` | Redis 风格 usage queue 协议和刷新通知 |
