...
func imagesMainModelBase(cfg *internalconfig.SDKConfig) string {
	if cfg != nil {
...
func (h *OpenAIAPIHandler) ImagesGenerations(c *gin.Context) {
	...
	responsesReq := buildImagesResponsesRequest(prompt, nil, tool, imagesMainModelBase(h.Cfg))
	if stream {
...
func (h *OpenAIAPIHandler) imagesEditsFromMultipart(c *gin.Context) {
	...
	responsesReq := buildImagesResponsesRequest(prompt, images, tool, imagesMainModelBase(h.Cfg))
	if stream {
...
func buildImagesResponsesRequest(prompt string, images []string, toolJSON []byte, mainModelBase string) []byte {
	req := []byte(`{"instructions":"","stream":true,"reasoning":{"effort":"medium","summary":"auto"},"parallel_tool_calls":true,"include":["reasoning.encrypted_content"],"model":"","store":false,"tool_choice":{"type":"image_generation"}}`)
	mainModel := strings.TrimSpace(mainModelBase)
	if mainModel == "" {
...
func (h *OpenAIAPIHandler) forwardImagesStream(ctx context.Context, c *gin.Context, flusher http.Flusher, cancel func(error), data <-chan []byte, errs <-chan *interfaces.ErrorMessage, firstChunk []byte, responseFormat string, streamPrefix string, writeEvent func(string, []byte)) {
	...
	keepAlive, _ := h.newImagesStreamKeepAliveTicker()
	defer func() {
		if keepAlive != nil {
			keepAlive.Stop()
		}
